...
> Status: **Proposed** · Owner: Nikolay Angelov · Last updated: 2026-06-27
...
|---|---|---|
| LLM | Llama-3.3-70B via HF `text-generation` (no tool calls) | OSS instruct model with **native tool-calling** via HF Inference Providers (OpenAI-compatible endpoint) |
| LLM reliability | single endpoint, no fallback | **Multi-LLM failover router** — timeout/health-aware fallback across several models/providers (see §6.6) |
...
| Deploy | HF Spaces (single Docker) | **HF Spaces** Docker, external managed datastores |

### Candidate OSS models (must support tool/function calling)
- `meta-llama/Llama-3.3-70B-Instruct` (tool-use via chat template)
- `Qwen/Qwen2.5-72B-Instruct` — strong function-calling
- `mistralai/Mistral-Small-...` / `mistralai/Mixtral` family
...
|---|---|---|
| **Guest login** | One-click anonymous session; full chat + job search + PDP; **no history saved** (Redis only, TTL). Upgrade-to-account preserves current session. | Redis |
...
- **Learned memories** (Postgres `user_memories`, **pgvector — internal/in-DB, no external service** [DECIDED]) — free-form facts the system infers: "prefers bullet points", "targeting product management in fintech", "dislikes generic advice", "based in Berlin". Embedded and retrieved by similarity to the current turn.
...
**Build vs library** — see decision §6.7. Default recommendation: **build on the pgvector infra we already have** (custom recall/learn step), optionally adopting an OSS memory library (mem0 / LangMem / Letta / Zep) if it earns its keep.
...
1. **Orchestration framework → LangGraph (recommended).** Model-agnostic, so it runs on HF OSS models; gives typed shared state, branching, retries, and streaming for the multi-agent graph. Alternative: a hand-rolled async orchestrator (fewer deps, more code). *Pick before Phase 1.*
...
3. **Embeddings model** — an OSS model (e.g. `BAAI/bge-*` / `intfloat/e5-*`). **Budget default: run `sentence-transformers` in-process** (no API cost, §11) rather than a hosted endpoint. Dimensions fix the pgvector column size — decide early.
...
5. **Datastores → Postgres + Redis only. [DECIDED — budget plan]** No separate MongoDB; Postgres (pgvector + JSONB) absorbs its role (§4). Two datastores, two free tiers / two local containers. Connection strings via Space secrets. Mongo remains addable later behind `repositories/` but is out of scope.
...
- **Failover order**, e.g. primary OSS model → secondary OSS model → alternate provider/endpoint of the same model. All entries must support native tool-calling so the agent loop is unaffected.
...
- **Streaming caveat:** failover is clean before the first token; once tokens have streamed to the client, fall back only if the stream breaks early (restart vs resume is a UX choice — default: restart with a brief "switching models" notice).
...
**Decision: custom on internal pgvector. [DECIDED]** Recall + learn implemented in our own `memory/` module over the `user_memories` table — no external memory service, no extra dependency, full control over what's learned and one-call GDPR delete. Options weighed below for the record.

| Option | What | Trade-off |
|---|---|---|
| **Custom on pgvector → CHOSEN** | Our own recall + learn step over a `user_memories` table | No new infra (pgvector already in stack), full control over what's learned and GDPR delete; we write the extraction prompt. Most flexible, modest build effort |
...
| **LangMem** (LangChain) | Memory utilities that fit LangGraph | Natural with LangGraph; couples us harder to the LangChain ecosystem |
...
If the custom extraction/dedup later proves fiddly, **mem0/LangMem** are fallback options; Letta/Zep stay rejected (extra services fight the HF-Spaces single-container constraint).

*Open questions for you:* (a) do guests get rate-limited harder than logged-in users? (assumed yes.) (b) failover scope — only across free OSS providers, or also a paid provider as a last resort? (c) on mid-stream failure, restart or resume? (d) should learned memories require user confirmation before they're applied, or be applied silently but always viewable/deletable?
...
| Risk | Mitigation |
|---|---|
| HF Spaces can't host 3 DBs | External managed services; Space holds only the app container + secrets |
...
|---|---|---|
| **LLM inference** | HF Inference free allowance; **failover list made of free providers** (HF Serverless, Groq free, OpenRouter `:free` models, Together credits) — the §6.6 router *is* the cost strategy | A paid endpoint as last failover entry |
| **Embeddings** | **`sentence-transformers` in-process** (e.g. `bge-small`/`e5-small`) — runs in the container, no API cost; sets the pgvector dimension | Hosted embedding API |
| **Auth** | **Backend SSO** (Authlib + Google/LinkedIn OIDC), free; no password storage | Clerk / hosted IdP (paid) |
| **Postgres + pgvector** (users/sessions/docs via JSONB + RAG/memory) | Neon/Supabase free tier, or self-host in Docker (local/VPS) | Paid tier at scale |
| **Redis** | Upstash free tier, or self-host | Paid tier |
...
**Consolidated datastores [DECIDED]:** **Postgres + Redis only** — not a separate MongoDB. Postgres covers everything stateful: **pgvector** for RAG + memories, **JSONB** for user/session/conversation/feedback documents (the role Mongo would play), and relational tables for jobs/PDP/dashboard. Redis stays for cache, streaming state, Celery broker, and rate limits. *Rationale (not cost — Mongo CE/Atlas M0 are free too):* on HF Spaces a Space is a **single, ephemeral container**, so stateful DBs must be external regardless; one external Postgres beats two services to provision and monitor. The `repositories/` layer keeps a future Mongo addition possible without touching services, but it's out of scope. See §4.
