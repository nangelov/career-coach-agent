...
> Status: **Proposed** · Last updated: 2026-06-27
...
- [ ] Confirm open decisions from design §6: orchestration = **LangGraph**, auth = **SSO (Google+LinkedIn) backend-owned [decided §6.2]**, embedding model + **vector dimension**, guest rate-limit policy, **LLM failover scope** (§6.6 open questions).
- [ ] Pick the **OSS tool-calling model** and verify native `tools=[...]` works via HF Inference Providers (OpenAI-compatible) with a throwaway script.
...
- [ ] Postgres + **alembic** migrations — **identity/docs (JSONB):** `users`, `profiles`, `preferences`, `sessions`, `conversations`/`messages`, `message_feedback`, `feedback`; **knowledge/vectors:** `kb_documents`, `kb_chunks(embedding vector(N))`, `user_memories(embedding vector(N))`; **structured:** `jobs`, `pdps`, dashboard (`goals`/`milestones`/`tasks`/`progress_entries`).
- [ ] `EmbeddingClient` (HF) + pgvector write/similarity-search helpers.
...
- [ ] `memory/` module: **recall** step (explicit prefs + top-k `user_memories` → context) wired into the graph before the planner.
- [ ] **Learn** step as a **Celery task** post-turn: extract durable preferences, dedup/update, confidence; thumb-down demotes/removes.
...
- [ ] Wire **external managed datastores** (Neon/Supabase Postgres+pgvector, Upstash Redis) via Space secrets.
...
- **LLM:** HF Inference free allowance; **failover across free providers** (HF / Groq / OpenRouter free models / Together credits) doubles as the budget strategy — no paid fallback unless you opt in.
- **Embeddings:** run a small **`sentence-transformers` model in-process** (free, no API) instead of a paid embedding endpoint.
- **Auth:** **backend SSO** (Authlib + Google/LinkedIn OIDC) — free, no password storage, no extra service.
- **Datastores [decided]:** **Postgres + Redis only** — free tiers (Neon/Supabase + Upstash) on Spaces, or self-hosted via `docker-compose` locally/VPS. Postgres JSONB + pgvector absorbs Mongo's role; rationale is single-container simplicity on Spaces, not cost (§4 / §11).
...
1. Orchestration: **LangGraph** vs hand-rolled async orchestrator.
2. Final **OSS model** for tool-calling (Llama-3.3-70B / Qwen2.5-72B / Mistral) + **embedding model & vector dimension** (default: in-process sentence-transformers).
3. **Datastores [decided]:** Postgres + Redis only (Mongo consolidated into Postgres JSONB) — confirm free managed tiers (Neon/Supabase + Upstash) vs self-host.
4. LLM failover provider list (which free providers) + guest rate-limit policy.
5. Teachable memory: **custom pgvector** vs library (mem0/LangMem).
