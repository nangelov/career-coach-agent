...
#: Chunking bounds for the embedded ``KbChunk`` rows. CVs are short, so a ~1k-char window with
#: a small overlap keeps each chunk semantically coherent while preserving context across
#: boundaries — a deliberately simple size-bounded splitter (no external splitter dependency;
#: none exists in the codebase yet).
