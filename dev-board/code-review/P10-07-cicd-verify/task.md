# Task P10-07-cicd-verify — P10 CI/CD verification (T)

- **Phase:** P10   **Status:** ENG   **Tags:** (T)

## Scope
Design/plan.md P10 exit: "CI/CD verification — run the full backend + frontend CI command sets
...
1. Read `.github/workflows/backend-ci.yml` and `.github/workflows/frontend-ci.yml`, extract the
   exact command sequence each job runs (including any curated-dependency-list step per
   P8-08-curated-deps-guard).
2. Run every command locally, exactly as CI does (same working directory, same flags).
3. Fix any drift caused by the P10-01..P10-06 work: new dependencies not yet in the curated list,
...
- [ ] Any new backend dependency introduced during P10 is present in the curated CI dependency
      list (P8-08 guard) and CI's pinned/curated venv build succeeds.
- [ ] `engineer.md` pastes the actual command output/summary for each of the 7 checks above.

## Design references
- dev-board/plan.md — P10 exit criterion; P8-08-curated-deps-guard precedent for the curated-deps
  check.
